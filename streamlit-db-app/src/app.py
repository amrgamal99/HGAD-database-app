...
def _format_summary_number(value) -> str:
    try:
        f = float(value)
        if pd.isna(f):
            return ""
...
def _render_supplier_costs_summary(df: pd.DataFrame) -> List[Tuple[str, str]]:
    """Summarize supplier costs by material name and add 2026 totals."""
    if df is None or df.empty:
        return []
...
def _render_supplier_name_summary(df: pd.DataFrame) -> List[Tuple[str, str]]:
    """Summarize supplier costs by supplier name and add 2026 totals."""
    if df is None or df.empty:
        return []

    amount_col = "القيمة خلال الفترة"
    date_col = "من تاريخ"
    supplier_col = next((c for c in ["اسم المورد", "اسم مورد"] if c in df.columns), None)

    if not supplier_col or amount_col not in df.columns:
        return []

    out = df.copy()
    out[amount_col] = pd.to_numeric(out[amount_col], errors="coerce")

    unique_suppliers = out[supplier_col].dropna().astype(str).str.strip().unique()
    ...
    for supplier in unique_suppliers:
        ...
        if not supplier_str or supplier_str.lower() in ("nan", "none"):
            continue

        supplier_sum = out.loc[out[supplier_col].astype(str).str.strip() == supplier_str, amount_col].sum(skipna=True)
        if not pd.isna(supplier_sum) and supplier_sum != 0:
            rows.append((supplier_str, _format_summary_number(supplier_sum)))

        if not df_2026.empty:
            supplier_sum_2026 = df_2026.loc[df_2026[supplier_col].astype(str).str.strip() == supplier_str, amount_col].sum(skipna=True)
            if not pd.isna(supplier_sum_2026) and supplier_sum_2026 != 0:
                rows.append((f"{supplier_str} 2026", _format_summary_number(supplier_sum_2026)))
...
def _render_dataframe_summary(df: pd.DataFrame, title: str = "الملخص الاحترافي", data_type: str = "") -> None:
    ...
    if data_type == "supplier_costs":
        ...
        supplier_pairs = _render_supplier_name_summary(df)
    else:
    ...
    if not summary_pairs and not supplier_pairs:
        return

    if summary_pairs:
        left_items, right_items = _split_pairs_two_columns(summary_pairs)
        st.markdown(f'<h3 class="hsec">{title}</h3>', unsafe_allow_html=True)
        fin_panel_two_tables(left_items=left_items, right_items=right_items)

    if supplier_pairs:
        left_items, right_items = _split_pairs_two_columns(supplier_pairs)
        st.markdown('<h3 class="hsec">ملخص الموردين</h3>', unsafe_allow_html=True)
        fin_panel_two_tables(left_items=left_items, right_items=right_items)
