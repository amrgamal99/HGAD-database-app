...
/* === Date inputs: make popover clickable & clear === */
[data-testid="stDateInput"] input {
    background:#0f172a !important; color:#e5e7eb !important;
    border:1px solid #334155 !important; border-radius:10px !important;
    text-align:center !important; height:44px !important; min-width: 160px !important;
}
[data-testid="stDateInput"] label { color:#cbd5e1 !important; font-weight:700; }
[data-testid="stSidebar"] * { overflow: visible !important; } /* allow popover outside */
div[role="dialog"], .stDateInput, .stDateInput > div[aria-modal="true"] { z-index: 99999 !important; }
.css-1o7jrs8, .css-1n76uvr, .stPopover { z-index: 99999 !important; }
...
/* === Two-column financial panel (right->left) === */
...
def _auto_excel_sheet(writer, df: pd.DataFrame, sheet_name: str):
    """Write a DataFrame into a nicely formatted sheet."""
...
def _format_numbers_for_display(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in out.columns:
        if pd.api.types.is_numeric_dtype(out[c]):
            ...
            # try to detect numeric-looking strings
            def _fmt_cell(v):
                s = str(v)
                try:
                    if s.strip().endswith("%"):
...
def make_pdf_bytes(df: pd.DataFrame, pdf_name: str = "", max_col_width: int = 180) -> bytes:
    """Generic single-table PDF (df should already be formatted)."""
    buf = BytesIO()
...
def make_pdf_combined(summary_df: pd.DataFrame, flow_df: pd.DataFrame, header_text: str = "") -> bytes:
    """One PDF file: summary (table) + page break + flow (table)."""
    buf = BytesIO()
    font_name, arabic_ok = register_arabic_font()
...
def fin_panel_two_tables(left_items, right_items):
    ...
    Each item is (label, value). We render cells as: [value | label]
    """
...
def main() -> None:
    ...
    if type_key == "financial_report":
        ...
        # Header line: Company | Project | Contract Date
        ...
        # ======= Two-column financial panel (no KPI cards) =======
        def _fmt(v):
            try:
        ...
        # Right table (as your screenshot – labels on the right, value on the left)
        right_items = [
        ...
        # Left table – you can wire real formulas later; placeholders shown
        left_items = [
            ("مواد أولية", _fmt(row.get("حجم الاعمال المنفذة", 0))),  # مثال: غيّر حسب معادلتك
        ...
        # clearer PDF (IDs removed, numbers formatted)
